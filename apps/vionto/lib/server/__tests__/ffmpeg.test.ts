...
describe("buildRenderCommand", () => {
  ...
  it("includes narration and music inputs when both are materialized", () => {
    ...
    expect(final).toContain("[1:a]volume=1.0[narration];[2:a]volume=0.22[music];[narration][music]amix=inputs=2:duration=first:dropout_transition=3:normalize=0[aout]");
