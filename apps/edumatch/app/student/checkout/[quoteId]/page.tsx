...
export default function CheckoutPage({ params }: { params: { quoteId: string } }) {
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-8">
        <div className="rounded-xl border border-red-200 bg-red-50 p-6">
          <h1 className="text-lg font-semibold text-red-800 mb-2">Checkout Error</h1>
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      <h1 className="text-2xl font-bold text-[var(--color-text)] mb-6">Complete Your Booking</h1>

      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-panel)] p-6 mb-6">
        <h2 className="text-lg font-semibold text-[var(--color-text)] mb-4">Booking Summary</h2>
        ...
        <div className="space-y-3 mb-6">
          ...
          <div className="flex justify-between">
            <span className="text-[var(--color-text-muted)]">Tutor</span>
          ...
          <div className="flex justify-between">
            ...
            <span className="font-medium text-[var(--color-text)]">
              €{(quote.hourlyRateCents / 100).toFixed(2)} / hour
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-[var(--color-text-muted)]">Duration</span>
            <span className="font-medium text-[var(--color-text)]">{quote.estimatedHours} hours</span>
          </div>
        </div>
        ...
        <div className="border-t border-[var(--color-border)] pt-4 space-y-2">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-[var(--color-text-muted)]">Tutor Receives</span>
      ...
      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-panel)] p-6">
        <h2 className="text-lg font-semibold text-[var(--color-text)] mb-4">Payment</h2>

        <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
          <p className="text-sm text-amber-800">
            <strong>Stripe Integration Required:</strong> To process real payments, integrate Stripe Elements here.
            The backend PaymentIntent is already created with id: <code className="text-xs bg-amber-100 px-1 rounded">{clientSecret?.slice(0, 20)}...</code>
          </p>
        </div>
        ...
        <div className="space-y-3 mb-6">
          ...
          <div className="p-3 border border-[var(--color-border)] rounded-lg bg-[var(--color-surface)] text-[var(--color-text-muted)] text-sm">
            💳 Card number field would appear here with Stripe Elements
          </div>
        </div>
        ...
        <p className="mt-4 text-xs text-center text-[var(--color-text-muted)]">
          Payments are processed securely by Stripe. EduMatch charges a 15% platform fee.
        </p>
      </div>
    </div>
  );
