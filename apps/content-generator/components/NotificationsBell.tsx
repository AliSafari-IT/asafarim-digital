...
export function NotificationsBell() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-[min(20rem,calc(100vw-1rem))] max-w-[calc(100vw-1rem)] rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-lg">
              ...
              notifications.map((n) => (
                <li key={n.id} className={`px-4 py-3 hover:bg-[var(--color-surface-elevated)] ${n.read ? "opacity-60" : ""}`}>
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                    ...
                    <span className="text-[10px] text-[var(--color-text-secondary)] sm:shrink-0">{n.when}</span>
                  </div>
