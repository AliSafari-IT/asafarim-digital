...
interface AppCardProps {
  ...
  description: string;
  href: string;
  status: "operational" | "degraded" | "down" | "coming-soon";
  icon: string;
}

export function AppCard({ name, description, href, status, icon }: AppCardProps) {
  const isActive = status === "operational";

  return (
    <a
      href={isActive ? href : undefined}
      className={`group block rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-elevated)] p-5 transition-all ${
        isActive
          ? "hover:border-[var(--color-primary)] hover:shadow-md cursor-pointer"
